...
class SparseStochMat:
    ...
    smaller CSR scipy sparse matrix containing the these rows/columns.
    ...
    @classmethod
    def from_full_csr_matrix(cls,
                             Tcsr: csr_matrix,
                             nz_rowcols: NDArray | None = None,
                             diag_val: float = 1.0) -> SparseStochMat:
        ...
        if nz_rowcols is None:
            nz_rows, nz_cols = (
                Tcsr - diag_val * eye(Tcsr.shape[0], format="csr")
            ).nonzero()
    ...
    def create_diag(cls,
        ...
        return cls.from_small_csr_matrix(size,
                                         T_small,
                                         np.array([]),
                                         diag_val=diag_val)
    ...
    def inplace_row_normalize(self, row_sum: float = 1.0):
        ...
        self.T_small.indices = self.T_small.indices.astype(np.int64,
                                                           copy=False)
    ...
    def set_to_zeroes(self,
        ...
        if self.T_small.data.size > 0:
            if relative:
                tol = tol * np.abs([self.T_small.data.min(),
                                    self.T_small.data.max()]).max()
    ...
    def sub_diag(self, diag_val=1.0):
        ...
        self - diag(diag_val)
        """
        return SparseStochMat.from_small_csr_matrix(
            self.size,
            self.T_small - diag_val*eye(self.T_small.shape[0], format="csr"),
            self.nz_rowcols,
            self.diag_val-diag_val
        )
    ...
    def check_nz_intersect_len(self, B):
        """
        Returns the length of the intersection of self.nz_rowcols & B.nz_rowcols
        """
    ...
    def __repr__(self):
        ...
        return f"{self.size}x{self.size} stochastic sparse matrix with " \
               "T_small:\n" + \
    ...
    def __matmul__(self, B):
        ...
        """
        if isinstance(B, SparseStochMat):
            # We split the problem: A @ B = (A-aI) @ (B-bI) + b*A + a*B - a*b*I
            ...
            if len(interset) == 0:
                ...
                ia_col = 0
                ic_col = 0
                # map col in A to col in C
                for ia_col in range(self.nz_rowcols.size):
                ...
                ic_col = 0
                # map col in A to col in C
                for ib_col in range(B.nz_rowcols.size):
                ...
                # C values will be on rows of Anz_rowcols and columns of
                # Bnz_rowcols

                # build two small_size x small_size matrices for (A-aI) and
                # (B-bI) in the subspace of Csmall and take their product
                AmI_small = SparseStochMat.from_small_csr_matrix(
                    small_size,
                    self.T_small - self.diag_val * eye(self.T_small.shape[0],
                                                       format="csr"),
                    Acol_to_Ccol,
                    diag_val=0.0
                )

                BmI_small = SparseStochMat.from_small_csr_matrix(
                    small_size,
                    B.T_small - B.diag_val * eye(B.T_small.shape[0],
                                                 format="csr"),
                    Bcol_to_Ccol,
                    diag_val=0.0
                )

                CmI = SparseStochMat.from_small_csr_matrix(
                    self.size,
    ...
    def __rmatmul__(self, s2):
        ...
        simply convert to full size sparse mat and perform operation.
        """
...
def inplace_csr_row_normalize(X, row_sum=1.0):
    ...
    if isinstance(X, SparseStochMat):
        ...
        if isspmatrix_csc(X):
            print(
                "Warning: row normalization on a CSC matrix will "
                "normalize columns."
            )
        ...
        if isinstance(row_sum, float):
            _css.inplace_csr_row_normalize(X.data,
                                           X.indptr,
                                           X.shape[0],
                                           row_sum)
            ...
            _css.inplace_csr_row_normalize_array(X.data,
                                                 X.indptr,
                                                 X.shape[0],
                                                 row_sum)
        else:
...
def rebuild_nnz_rowcol(T_small: csr_matrix,
                       nonzero_indices: NDArray,
                       size: int,
                       diag_val: float = 1.0) -> csr_matrix:
    ...
    """
    (data, indices, indptr, n_rows) = _css.rebuild_nnz_rowcol(
        T_small.data,
...
def inplace_csr_matmul_diag(A: csr_matrix | csc_matrix,
                            diag_vec: NDArray) -> None:
    ...
    assert A.shape[1] == diag_vec.size, "Invalid array size: diag_vec must " \
                                        "match column count."
...
def _inplace_diag_matmul_csr(A: csr_matrix | csc_matrix,
                             diag_vec: NDArray) -> None:
...
def inplace_diag_matmul_csr(A: csr_matrix | csc_matrix,
                            diag_vec: NDArray) -> None:
    ...
    diag_vec = diag_vec.squeeze()
    assert diag_vec.ndim == 1
    assert A.shape[0] == diag_vec.size, "Invalid array size: diag_vec must " \
                                        "match row count."
...
class SparseAutocovMat:
    ...
    def __init__(self, PT: csr_matrix,
        ...
        if isinstance(p1, np.ndarray) and isinstance(p2, np.ndarray):
            ...
            assert PT.shape[0] == PT.shape[1] == p1.size == p2.size, \
                f"PT.shape[0]={PT.shape[0]}, PT.shape[1]={PT.shape[1]}, " \
                f"p1.size={p1.size}, p2.size={p2.size}"

            # Array Normalization Check
            assert np.isclose(p1.sum(), 1.0, atol=1e-5), \
                f"Array p1 is not normalized (sum = {p1.sum()})"
            assert np.isclose(p2.sum(), 1.0, atol=1e-5), \
                f"Array p2 is not normalized (sum = {p2.sum()})"

            self.p_scalars = False

        elif isinstance(
            p1, (float, int, np.number)
        ) and isinstance(p2, (float, int, np.number)):
            # Scalar Normalization Check (Mass = scalar * N)
            assert np.isclose(p1 * N, 1.0, atol=1e-5), \
                f"Scalar p1={p1} is not normalized for N={N}. Expected {1.0/N}"
            assert np.isclose(p2 * N, 1.0, atol=1e-5), \
                f"Scalar p2={p2} is not normalized for N={N}. Expected {1.0/N}"
    ...
    def __repr__(self):
        ...
        if self.PT_symmetric:
            return f"{self.size}x{self.size} sparse autocovariance matrix " \
                   f"with symmetric PT:\n" + self.PT.__repr__()
        else:
            return f"{self.size}x{self.size} sparse autocovariance matrix " \
                   f"with PT:\n" + self.PT.__repr__()
    ...
    def from_T(cls, T, p1=None, p2=None):
        ...
        T : NxN scipy csr matrix
            Transition matrix. T[i,j] is the prob to go from i to j between
            t1 and t2.
        p1 : numpy ndarray, optional
            Probability vector (size = N) at t1.
            Default is p1[i] = 1/N for all i.
    ...
    def get_element(self, i, j):
        ...
        k, = np.where(
            self.PT.indices[self.PT.indptr[i]:self.PT.indptr[i+1]] == j
        )
        if len(k) == 0:
            return -1*p0
        else:
    ...
    def aggregate(self, idx_list):
        ...
        idxs_array = np.array([i for idx in idx_list for i in idx],
                              dtype=np.int32)
        ...
        if new_size**2 < self.PT.data.size:
            PTdata, PTrows, PTcols, new_size = _css.aggregate_csr_mat(
                self.PT.data,
                self.PT.indices,
                self.PT.indptr,
                idxs_array,
            ...
            PTdata, PTrows, PTcols, new_size = _css.aggregate_csr_mat_2(
                self.PT.data,
                self.PT.indices,
                self.PT.indptr,
                idxs_array,
        ...
        newPT = coo_matrix((PTdata, (PTrows, PTcols)),
...
def sparse_outer(p, use_mkl=True, triu=True, verbose=False, log_message=""):
    ...
    if triu:
        ...
        num_per_row = [
            next(row_len) if r in p.indices else 0 for r in range(p.shape[1])
        ]
    else:
        indices_list = [p.indices]*p.indices.size
        num_per_row = [
            p.indices.size if r in p.indices else 0 for r in range(p.shape[1])
        ]
    ...
    return csr_matrix(
        (Odata, Oindices, Oindptr), shape=(p.shape[1], p.shape[1])
    )
