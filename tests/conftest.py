...
# Re-export shared test helpers so they are available to downstream packages
# via ``stochmat.testing`` and locally as fixtures.
from stochmat.testing import make_clusterings, make_equal_clusterings


@pytest.fixture(scope='session')
def fast_modules():
    """Provide both Cython and pure-Python ``fast`` modules side-by-side.
...
@pytest.fixture(scope='session')
def make_clusterings_fixture():
    """Expose :func:`stochmat.testing.make_clusterings` as a fixture."""
    return make_clusterings


@pytest.fixture(scope='session')
def make_equal_clusterings_fixture():
    """Expose :func:`stochmat.testing.make_equal_clusterings` as a fixture."""
    return make_equal_clusterings
...
def get_csr_matrix_large():
    ...
    if len(zero_rows) > 0:
        ...
        # Reconstruct matrix
        raw_matrix = csr_matrix((all_data, (all_row, all_col)),
                                shape=(size, size))
        # Recalculate sums
        row_sums = np.array(raw_matrix.sum(axis=1)).flatten()
    # Normalize rows
    ...
    coo = raw_matrix.tocoo()
    # Safe division: ensure no zeros remain
    # (though we added self-loops, so sums > 0)
    ...
    def _get_matrix(nbr: int = 1,
        ...
        for _ in range(nbr):
            ...
            if len(zero_indices) > 0:
                ...
                mat = MatrixType((all_data, (all_row, all_col)),
                                 shape=(size, size))
